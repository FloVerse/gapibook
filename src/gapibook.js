const useGoogleBookApi = {
  ...
  byEbookFree: (search) =>
    ...
    byNewest: (search) =>
    new Promise((resolve, reject) => {
      const BOOK_API_URL = `https://www.googleapis.com/books/v1/volumes?q=${search}&orderBy=newest`;
    ...
    byAuthor: (search,author) =>
    new Promise((resolve, reject) => {
      const BOOK_API_URL = `https://www.googleapis.com/books/v1/volumes?q=${search}+inauthor:${author}`;
